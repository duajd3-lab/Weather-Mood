...
프로젝트 **Weather Mood** 개발 과정에서 발생한 문제와 해결 과정을 정리했습니다.
...
- 비동기 데이터 처리 방식 이해 (`async / await`)
- API 데이터 구조 분석 및 객체 접근 방법
- `localStorage` / `sessionStorage` 상태 관리
- API 호출 최적화 및 요청 제한 대응
- UI 상태와 데이터 상태 동기화 경험
